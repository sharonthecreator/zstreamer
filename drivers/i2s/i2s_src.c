...
static int i2s_src_init(const struct device *dev) {
  ...
  LOG_INF("I2S source %s: %u Hz, %u-bit, %u ch, block %u B", dev->name,
          cfg->sample_rate_hz, cfg->word_size, cfg->num_channels,
          cfg->block_size);
...
#define I2S_SRC_DEFINE(inst)                                                   \
  ZSTREAMER_SOURCE_DT_INST_PRE_DEFINE(inst);                                   \
                                                                               \
  K_MEM_SLAB_DEFINE_STATIC(i2s_src_rx_slab_##inst,                             \
