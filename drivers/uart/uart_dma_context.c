 ...
 */
static struct uart_dma_context *find_or_create_context(const struct device *uart_dev)
{
 ...
 */
static void uart_dma_callback(const struct device *uart_dev, struct uart_event *evt,
			      void *user_data)
{
	...
	case UART_RX_RDY:
		if (ctx->rx_handler != NULL) {
			ctx->rx_handler(ctx->rx_user_data, evt->data.rx.buf + evt->data.rx.offset,
					evt->data.rx.len);
		}
		break;
	...
	case UART_RX_DISABLED:
		if (!ctx->rx_disabling && ctx->rx_handler != NULL) {
			...
			int err = uart_rx_enable(ctx->uart_dev, ctx->rx_buf, ctx->rx_buf_len,
						 ctx->rx_timeout);
			if (err != 0) {
