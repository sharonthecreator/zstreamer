...
static int uart_sink_process_dma(const struct device *dev, struct net_buf *buf)
{
	const struct uart_sink_config *cfg = dev->config;
	struct uart_sink_data *data = dev->data;
	int ret;
...
static int uart_sink_process_poll(const struct device *dev, struct net_buf *buf)
{
	const struct uart_sink_config *cfg = dev->config;
