...
static int src_uart_process_dma(const struct device *dev,
				   struct net_buf *buf)
{
	struct src_uart_data *data = dev->data;
...
static int src_uart_process(const struct device *dev,
			       struct net_buf *buf)
{
#if defined(CONFIG_UART_ASYNC_API)
	struct src_uart_data *data = dev->data;
