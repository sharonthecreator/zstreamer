...
#define ODD_FILTER_DEFINE(inst)                                                                    \
	...
	static const struct zstreamer_filter_config odd_filter_config_##inst =                     \
		ZSTREAMER_FILTER_CONFIG_INIT(inst, true);                                          \
	DEVICE_DT_INST_DEFINE(inst, zstreamer_filter_common_init, NULL, &odd_filter_data_##inst,   \
			      &odd_filter_config_##inst, POST_KERNEL,                              \
			      CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &odd_filter_api);
