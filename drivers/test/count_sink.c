...
#define COUNT_SINK_DEFINE(inst)                                                                    \
	...
	DEVICE_DT_INST_DEFINE(inst, zstreamer_sink_common_init, NULL, &count_sink_data_##inst,     \
			      &count_sink_config_##inst, POST_KERNEL,                              \
			      CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &count_sink_api);
