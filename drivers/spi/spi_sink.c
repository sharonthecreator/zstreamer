...
static int spi_sink_process_async(const struct device *dev, struct net_buf *buf)
{
...
static int spi_sink_open_async(const struct device *dev)
{
	...
	ret = spi_write_signal(cfg->spi.bus, &cfg->spi.config, &test_bufs, &data->sig);
	if (ret == -ENOTSUP) {
...
static int spi_sink_process_poll(const struct device *dev, struct net_buf *buf)
{
	const struct spi_sink_config *cfg = dev->config;
