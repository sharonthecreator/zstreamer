 ...
 * Note: Currently unused - will be enabled when DMA IRQ integration is
 * complete.
 */
static void __maybe_unused adc_dma_callback(bool half_complete) {
  ...
  if (half_complete) {
    ...
    buffer =
        (const uint8_t *)data->dma_buffer + (half_size * data->sample_size);
  }

  data->cfg.callback(data->cfg.user_data, buffer, data->cfg.buffer_samples,
                     data->cfg.num_channels);
}
...
static int configure_timer_trgo(TIM_TypeDef *timer, uint32_t sample_rate_hz) {
  ...
  if (period > 0xFFFF || period == 0) {
    LOG_ERR("Cannot achieve sample rate %u Hz (clock=%u)", sample_rate_hz,
            timer_clock);
    return -EINVAL;
  }
...
static int configure_adc(struct adc_src_stm32_data *data, ADC_TypeDef *adc,
  ...
  for (int i = 0; i < data->cfg.num_channels; i++) {
    uint32_t channel =
        __LL_ADC_DECIMAL_NB_TO_CHANNEL(data->cfg.adc_channels[i]);
  ...
  if (data->cfg.num_channels == 2) {
    uint32_t channel2 =
        __LL_ADC_DECIMAL_NB_TO_CHANNEL(data->cfg.adc_channels[1]);
    LL_ADC_REG_SetSequencerRanks(adc, LL_ADC_REG_RANK_2, channel2);
#if defined(CONFIG_SOC_SERIES_STM32U5X)
    LL_ADC_SetChannelSamplingTime(adc, channel2, LL_ADC_SAMPLINGTIME_20CYCLES);
#else
    LL_ADC_SetChannelSamplingTime(adc, channel2, LL_ADC_SAMPLINGTIME_COMMON_1);
#endif
  }
...
static int configure_dma_stm32u5(struct adc_src_stm32_data *data,
  ...
  LL_DMA_SetBlkHWRequest(GPDMA1, LL_DMA_CHANNEL_0,
  ...
  if (data->sample_size == 2) {
    LL_DMA_SetSrcDataWidth(GPDMA1, LL_DMA_CHANNEL_0,
                           LL_DMA_SRC_DATAWIDTH_HALFWORD);
    LL_DMA_SetDestDataWidth(GPDMA1, LL_DMA_CHANNEL_0,
  ...
  LOG_DBG("DMA configured: buffer=%p, size=%u bytes", data->dma_buffer,
          buffer_size);
...
int adc_src_stm32_init(struct adc_src_stm32_data *data,
  ...
  if (cfg->num_channels < 1 || cfg->num_channels > 2) {
    LOG_ERR("Invalid channel count: %u (must be 1 or 2)", cfg->num_channels);
    return -EINVAL;
  }
  ...
  LOG_INF("ADC source initialized: %u Hz, %u-bit, %u ch", cfg->sample_rate_hz,
