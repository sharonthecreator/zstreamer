...
static int adc_src_init(const struct device *dev)
{
	...
	LOG_INF("ADC source %s: %u Hz, %u ch, %u samples/buf, %u-bit", dev->name,
		cfg->sample_rate_hz, cfg->num_channels, cfg->samples, cfg->channels[0].resolution);
...
#define ADC_SRC_CHANNEL_SPEC(node_id, prop, idx) ADC_DT_SPEC_GET_BY_IDX(node_id, idx),
