...
struct fs_sink_data {
  ...
  bool file_opened;
  /**
   * True once file_index has been loaded from the persisted index file (or
   * explicitly set via fs_sink_reset).  Prevents re-reading the index on
   * every file open — only the very first process() call after boot loads
   * it.  Doesn't load the index in init as the filesystem might not be
   * mounted.  The index file is named "<prefix>idx" (e.g. "audio_idx").
   */
  bool index_loaded;
...
/*
 * Index persistence — the file_index is saved to <mount_path>/<prefix>idx
 * (e.g. "audio_idx") as a raw uint32_t so the driver can resume at the
 * correct file after a reboot.  Loaded lazily on the first process() call;
 * saved after every rotation.
 */

/** Build the path to the <prefix>idx persistence file. */
static int idx_path(const struct device *dev, char *buf, size_t buf_size) {
  const struct fs_sink_config *cfg = dev->config;
  int n = snprintf(buf, buf_size, "%s/%sidx", cfg->mount_path,
                   cfg->filename_prefix);
...
/**
 * Read file_index from the <prefix>idx file on disk.  Falls back to 0 if the
 * file is missing or corrupt.  Clamps to file_count if the saved value
 * exceeds the limit (e.g. file_count was reduced since the last boot).
 */
...
/** Persist the current file_index to the <prefix>idx file so it survives
 * reboot. */
...
static int open_new_file(const struct device *dev) {
  ...
  ret = data->filename_cb(dev, data->path_buf, sizeof(data->path_buf),
                          cfg->mount_path, cfg->filename_prefix,
                          data->file_index, data->filename_cb_user_data);
  if (ret < 0) {
 ...
 *
 * File opening is lazy: the first call loads the persisted index from the
 * <prefix>idx file and opens the initial file.  This avoids init-ordering
 * issues with the filesystem (LittleFS registers at POST_KERNEL priority 99,
 * well after device init at priority 40).  Subsequent calls check rotation
 * thresholds and write buffer data to the current file.
 */
static int fs_sink_process(const struct device *dev, struct net_buf *buf) {
  ...
  if (!data->file_opened) {
    ...
    /* Persist the index immediately so the index file exists on
     * disk even if the device loses power before the first
     * rotation. */
    ret = save_index(dev);
    if (ret != 0) {
      LOG_WRN("failed to persist initial index: %d", ret);
    }
 ...
 * next process() call reloads from the <prefix>idx file (or defaults to 0
 * if absent).
