...
static int dac_sink_init(const struct device *dev)
{
	...
	LOG_INF("DAC sink %s: %u ch, %u-bit", dev->name, cfg->num_channels, cfg->dac_resolution);
...
static int dac_sink_process(const struct device *dev, struct net_buf *buf)
{
	...
	for (size_t s = 0; s < num_samples; s++) {
		for (uint8_t ch = 0; ch < cfg->num_channels; ch++) {
			...
			if (ret < 0) {
				LOG_ERR("dac_write_value ch%u failed: %d", cfg->channel_ids[ch],
					ret);
				return ret;
			}
		}
...
#define DAC_SINK_CHANNEL_ELEM(node_id, prop, idx) DT_PROP_BY_IDX(node_id, prop, idx),

#define DAC_SINK_DEFINE(inst)                                                                      \
	ZSTREAMER_SINK_DT_INST_PRE_DEFINE(inst);                                                   \
                                                                                                   \
	static const uint8_t dac_sink_channels_##inst[] = {                                        \
		DT_INST_FOREACH_PROP_ELEM(inst, dac_channel_ids, DAC_SINK_CHANNEL_ELEM)};          \
                                                                                                   \
