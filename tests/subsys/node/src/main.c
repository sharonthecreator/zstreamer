...
ZTEST(zstreamer_node, test_byte_count)
{
	...
	zassert_equal(bytes, bufs * BUFFER_SIZE, "bytes %u != bufs %u * buffer_size %u", bytes,
		      bufs, BUFFER_SIZE);
...
ZTEST(zstreamer_node, test_high_throughput)
{
	...
	zassert_true(bufs >= 100, "expected >= 100 bufs through processor, got %u", bufs);
	zassert_equal(bytes, bufs * BUFFER_SIZE, "byte count %u != bufs %u * %u", bytes, bufs,
		      BUFFER_SIZE);
...
ZTEST(zstreamer_node, test_sustained_restart)
{
	...
	zassert_true(total_bufs >= 30, "expected >= 30 total bufs across 30 cycles, got %u",
		     total_bufs);
}
...
ZTEST(zstreamer_node, test_buffer_rate_matches_interval)
{
	...
	zassert_true(bufs <= expected + 1, "got %u bufs, exceeds expected %u", bufs, expected);
}
...
ZTEST(zstreamer_node, test_low_pipeline_overhead)
{
	...
	zassert_true(overhead_ms <= TICK_MS, "per-buffer overhead %u ms exceeds 1 tick (%u ms)",
		     overhead_ms, TICK_MS);
}
