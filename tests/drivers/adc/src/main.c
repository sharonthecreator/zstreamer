...
ZTEST(zstreamer_adc, test_src_16bit_produces_data)
{
	...
	zassert_true(bytes % sizeof(int16_t) == 0, "byte count %u not aligned to 16-bit samples",
		     bytes);
}
...
ZTEST(zstreamer_adc, test_src_16bit_frame_size)
{
	...
	zassert_equal(bytes / bufs, expected_frame, "frame size %u != expected %u", bytes / bufs,
		      expected_frame);
	zassert_equal(bytes % expected_frame, 0, "total bytes %u not a multiple of frame size %u",
...
ZTEST(zstreamer_adc, test_src_16bit_restart_cycle)
{
	...
	for (int cycle = 0; cycle < 5; cycle++) {
		...
		zassert_true(count_sink_get_buf_count(count_dev) > 0, "cycle %d: no buffers",
			     cycle);
...
ZTEST(zstreamer_adc, test_src_8bit_sample_size)
{
	...
	zassert_true(bytes_16 > bytes_8, "16-bit bytes (%u) should exceed 8-bit bytes (%u)",
		     bytes_16, bytes_8);
}
