...
ZTEST(zstreamer_fs, test_data_integrity)
{
	...
	for (ssize_t i = 0; i < nread; i++) {
		zassert_equal(buf[i], expected, "byte %zd: got 0x%02x expected 0x%02x", i, buf[i],
			      expected);
		expected++;
	}
...
ZTEST(zstreamer_fs, test_delta_rotation)
{
	...
	zassert_true(entry.size <= 256, "delta file 0 too large (%zu) — size rotation?",
		     entry.size);
...
static int custom_filename_cb(const struct device *dev, char *buf, size_t buf_size,
