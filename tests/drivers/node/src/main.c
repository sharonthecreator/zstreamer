...
ZTEST_SUITE(zstreamer_node_uart, NULL, NULL, cleanup, cleanup, NULL);

/* ------------------------------------------------------------------ */
/* Basic functional tests                                              */
/* ------------------------------------------------------------------ */

ZTEST(zstreamer_node_uart, test_devices_ready) {
...
ZTEST(zstreamer_node_uart, test_start_stop) {
  int ret;
...
ZTEST(zstreamer_node_uart, test_buf_alloc) {
  struct net_buf *buf;
...
ZTEST(zstreamer_node_uart, test_uart_relay) {
  ...
  zassert_equal(rx_len, tx_len, "expected %u bytes, got %u", tx_len, rx_len);
...
ZTEST(zstreamer_node_uart, test_long_transfer) {
  ...
  zassert_equal(rx_total, sizeof(tx_data), "long xfer: expected %u, got %u",
                (unsigned)sizeof(tx_data), rx_total);
  zassert_mem_equal(rx_buf, tx_data, sizeof(tx_data));
}

ZTEST(zstreamer_node_uart, test_burst_writes) {
  ...
  for (size_t i = 0; i < n_msgs; i++) {
    uart_emul_put_rx_data(uart_src_dev, (const uint8_t *)msgs[i], msg_len);
    k_msleep(50);
  }
  ...
  rx_total = uart_emul_get_tx_data(uart_sink_dev, rx_buf, sizeof(rx_buf));
  zassert_equal(rx_total, n_msgs * msg_len, "burst: expected %u, got %u",
                (unsigned)(n_msgs * msg_len), rx_total);
  zassert_mem_equal(rx_buf, expected, n_msgs * msg_len);
}

ZTEST(zstreamer_node_uart, test_restart_cycle) {
  ...
  for (int cycle = 0; cycle < 5; cycle++) {
    ...
    rx_len = uart_emul_get_tx_data(uart_sink_dev, rx_buf, sizeof(rx_buf));
    zassert_equal(rx_len, 2, "cycle %d: expected 2, got %u", cycle, rx_len);
    zassert_mem_equal(rx_buf, pattern, 2);
  }
}

ZTEST(zstreamer_node_uart, test_throughput_stress) {
  /*
  ...
  rx_total = uart_emul_get_tx_data(uart_sink_dev, rx_buf, sizeof(rx_buf));
  zassert_equal(rx_total, sizeof(tx_data), "stress: expected %u, got %u",
                (unsigned)sizeof(tx_data), rx_total);
  zassert_mem_equal(rx_buf, tx_data, sizeof(tx_data));
}
...
static void run_large_transfer(uint32_t total, uint32_t chunk) {
  ...
  while (tx_pos < total || rx_pos < total) {
    ...
    if (tx_pos < total) {
      ...
      uint32_t sent = uart_emul_put_rx_data(uart_src_dev, tx_buf, want);
      tx_pos += sent;
    }
    ...
    for (;;) {
      ...
      uint32_t got = uart_emul_get_tx_data(uart_sink_dev, rx_buf, want);
      if (got == 0) {
        break;
      }
      ...
      for (uint32_t i = 0; i < got; i++) {
        ...
        zassert_equal(rx_buf[i], exp, "mismatch at byte %u: 0x%02x != 0x%02x",
  ...
  zassert_equal(rx_pos, total, "total mismatch: expected %u, got %u", total,
                rx_pos);
}

ZTEST(zstreamer_node_uart, test_1mb_transfer) {
  /* 1 MiB streamed in 4 KiB chunks — every byte verified. */
  run_large_transfer(1024u * 1024u, 4096u);
}

ZTEST(zstreamer_node_uart, test_2mb_varied_chunks) {
  /*
  ...
  zassert_equal(rx_pos, total, "total: expected %u, got %u", total, rx_pos);
}
...
static void numgen_cleanup(void *fixture) {
  ...
  zstreamer_source_stop(numgen_src_dev);
}

ZTEST_SUITE(zstreamer_node_test, NULL, NULL, numgen_cleanup, numgen_cleanup,
            NULL);
...
ZTEST(zstreamer_node_test, test_numgen_fakesink_start_stop) {
  ...
  /* Source start/stop. */
  ret = zstreamer_source_start(numgen_src_dev);
  zassert_true((ret == 0) || (ret == -EALREADY), "numgen start unexpected: %d",
               ret);

  /* Starting again should report already running on stable targets. */
  ret = zstreamer_source_start(numgen_src_dev);
  zassert_true((ret == 0) || (ret == -EALREADY), "double start unexpected: %d",
               ret);

  ret = zstreamer_source_stop(numgen_src_dev);
  zassert_true((ret == 0) || (ret == -EALREADY), "numgen stop unexpected: %d",
               ret);

  /* Stopping again should report already stopped on stable targets. */
  ret = zstreamer_source_stop(numgen_src_dev);
  zassert_true((ret == 0) || (ret == -EALREADY), "double stop unexpected: %d",
               ret);
}
...
ZTEST(zstreamer_node_test, test_numgen_fakesink_restart_cycle) {
  for (int cycle = 0; cycle < 5; cycle++) {
    ...
    (void)zstreamer_source_stop(numgen_src_dev);

    ret = zstreamer_source_start(numgen_src_dev);
    zassert_true((ret == 0) || (ret == -EALREADY), "cycle %d numgen start: %d",
                 cycle, ret);
    ...
    ret = zstreamer_source_stop(numgen_src_dev);
    zassert_true((ret == 0) || (ret == -EALREADY), "cycle %d numgen stop: %d",
                 cycle, ret);
  }
}
