...
ZTEST(zstreamer_node_spi, test_spi_relay) {
   ...
   */
  zassert_true(rx_len >= tx_len, "expected at least %u bytes, got %u", tx_len,
               rx_len);
...
ZTEST(zstreamer_node_spi, test_spi_restart_cycle) {
  ...
  for (int cycle = 0; cycle < 5; cycle++) {
    ...
    rx_len = spi_test_get_tx_data(spi_tx_dev, rx_buf, sizeof(rx_buf));
    zassert_true(rx_len >= sizeof(pattern), "cycle %d: expected >= %u, got %u",
...
ZTEST(zstreamer_node_spi, test_spi_large_verified) {
  ...
  for (size_t i = 0; i < sizeof(tx_data); i++) {
    zassert_equal(rx_data[i], tx_data[i],
                  "mismatch at byte %u: 0x%02x != 0x%02x", (unsigned)i,
                  rx_data[i], tx_data[i]);
  }
}
