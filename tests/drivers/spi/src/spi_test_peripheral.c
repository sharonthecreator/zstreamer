 ...
 * zstreamer_node SPI source/sink drivers on native_sim.
 *
...
static int spi_test_emul_io(const struct emul *target, const struct spi_config *config,
			    const struct spi_buf_set *tx_bufs, const struct spi_buf_set *rx_bufs)
{
	...
	if (rx_bufs != NULL) {
		for (size_t i = 0; i < rx_bufs->count; i++) {
			...
			if (b->buf != NULL && b->len > 0) {
				uint32_t got = ring_buf_get(&data->rx_ring, b->buf, b->len);
				/* Zero-fill if not enough data. */
				if (got < b->len) {
					memset((uint8_t *)b->buf + got, 0, b->len - got);
				}
			}
		}
	}
...
static int spi_test_emul_init(const struct emul *target, const struct device *parent)
{
	ARG_UNUSED(target);
	ARG_UNUSED(parent);
	return 0;
...
#define SPI_TEST_PERIPHERAL_DEFINE(inst)                                                           \
	...
	DEVICE_DT_INST_DEFINE(inst, spi_test_dev_init, NULL, &spi_test_data_##inst, NULL,          \
			      POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE, NULL);              \
	EMUL_DT_INST_DEFINE(inst, spi_test_emul_init, &spi_test_data_##inst, NULL,                 \
