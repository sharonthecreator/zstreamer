...
static const struct device *dac_sink_multi_dev = DEVICE_DT_GET(DAC_SINK_MULTI_NODE);
...
static const struct device *fake_dac_multi_dev = DEVICE_DT_GET(FAKE_DAC_MULTI_NODE);
...
ZTEST(zstreamer_dac, test_16bit_values_in_range)
{
	...
	for (uint32_t i = 0; i < n; i++) {
		...
		zassert_true(val <= 4095, "sample %u: value %u out of 12-bit range", i, val);
	}
}
...
ZTEST(zstreamer_dac, test_16bit_data_integrity)
{
	...
	zassert_true(start >= 0, "could not derive byte offset from first=%u second=%u", first,
		     second);
	...
	for (uint32_t i = 0; i < n; i++) {
		...
		zassert_equal(actual, expected, "sample %u: DAC %u != expected %u (lo=%u hi=%u)", i,
...
ZTEST(zstreamer_dac, test_8bit_data_integrity)
{
	...
	for (uint32_t i = 0; i < n; i++) {
		...
		zassert_equal(actual, expected, "sample %u: DAC %u != expected %u", i, actual,
			      expected);
	}
}
...
ZTEST(zstreamer_dac, test_8bit_more_samples_than_16bit)
{
	 ...
	 */
	zassert_true(w_8 > w_16, "8-bit writes (%u) should exceed 16-bit writes (%u)", w_8, w_16);
}
...
ZTEST(zstreamer_dac, test_multi_channel_alternates)
{
	...
	for (uint32_t i = 0; i < n; i++) {
		...
		zassert_equal(actual_ch, expected_ch, "write %u: channel %u != expected %u", i,
