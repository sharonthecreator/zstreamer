...
static int fake_dac_write_value(const struct device *dev, uint8_t channel, uint32_t value)
{
...
#define FAKE_DAC_DEFINE(inst)                                                                      \
	...
	DEVICE_DT_INST_DEFINE(inst, fake_dac_init, NULL, &fake_dac_data_##inst, NULL, POST_KERNEL, \
			      CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &fake_dac_api);
