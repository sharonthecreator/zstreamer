...
static void fake_child_init(struct fake_child *child, const char *name,
                            bool readonly) {
...
ZTEST(zstreamer_distribute, test_two_readwrite_clones_first) {
  ...
  zassert_equal_ptr(got_b, orig,
                    "last rw child should get original buffer (ref)");
...
ZTEST(zstreamer_distribute, test_clone_data_integrity) {
  ...
  zassert_equal(clone->len, BUFFER_SIZE, "clone len %u != %u", clone->len,
                BUFFER_SIZE);
  zassert_equal(ref->len, BUFFER_SIZE, "ref len %u != %u", ref->len,
                BUFFER_SIZE);
  zassert_mem_equal(clone->data, ref->data, BUFFER_SIZE,
                    "clone data differs from original");

  /* Verify the known pattern. */
  for (size_t i = 0; i < BUFFER_SIZE; i++) {
    zassert_equal(clone->data[i], 0xAB, "clone byte %zu = 0x%02x", i,
                  clone->data[i]);
  }
...
ZTEST(zstreamer_distribute, test_readonly_then_readwrite) {
  ...
  /* Data still matches. */
  zassert_mem_equal(got_b->data, got_a->data, BUFFER_SIZE,
                    "clone data differs from ref");
...
ZTEST(zstreamer_distribute, test_readwrite_then_readonly) {
  ...
  zassert_not_equal((uintptr_t)got_a, (uintptr_t)orig,
                    "first rw child should get a clone");
...
ZTEST(zstreamer_distribute, test_two_readwrite_extra_ref) {
  ...
  /* Data integrity. */
  zassert_mem_equal(got_a->data, got_b->data, BUFFER_SIZE,
                    "clone data mismatch");
